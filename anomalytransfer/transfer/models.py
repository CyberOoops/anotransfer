...
def sbd_2(a, b) -> float:
    ...
    l2_b = np.linalg.norm(b)
    cross_correlation = np.convolve(a, b, mode='full')
    return 1 - np.max(cross_correlation) / (l2_a * l2_b)
def sbd_3(a, b) -> float:
    a = np.asarray(a, dtype=np.float32).reshape(-1)
    b = np.asarray(b, dtype=np.float32).reshape(-1)
    ...
    for i in range(0,len(a)-1):
        b_shfit = np.append(b[i:],b[:i])
        s = 1 - np.dot(a,b_shfit)/(l2_a * l2_b)
        #print(s)
        if(s<s_min):
            s_min=s
            pace_shift = i
    print(s_min,pace_shift)
    return pace_shift,  s_min
