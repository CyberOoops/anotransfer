...
def group_data_by_weekday(timestamps: Sequence, values: Sequence) -> Tuple[Sequence, Sequence]:
    ...
    for i in range(len(weekday)):
        if weekday[i] != current_weekday:
            if current_index != 0 and len(values[(current_index):i])!=0:
                # ! Add more 119 points (Bagel ignore the first 119 points!)
                grouped_data[current_weekday].append(values[(current_index):i])
                grouped_ts[current_weekday].append(timestamps[(current_index):i])
