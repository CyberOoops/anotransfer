...
def make_base_model(kpi: at.transfer.data.KPI, epochs: int, cluster):
    ...
    if sys.argv[1]=='Yahoo':
        ...
        window=120
    model = at.transfer.models.AnomalyDetector(window_size=window)
...
def get_delay_test_results(
    ...
    #         pre = precision
    #         rec = recall
    f1,pre,rec,predict = at.utils.delay_f1(adjusted_scores,adjusted_labels,k)

    print('f_score{} precison{} recall{}'.format(f1,pre,rec))
    # with open('all_result.txt','a') as f:
    #     f.write('%f %f %f\n'%(f1,pre,rec))
    return f1,pre,rec
...
def main(finetune_num=200):
    ...
    for file in file_list:
        ...
        # cluster_value, finetune_value = data_kpi.split_by_idx(1440, window_size=1)
        # unuse_value, test_value = data_kpi.split_by_idx(int(len(data_kpi.timestamps)*0.6)-finetune_num, window_size=1)
        ...
        if sys.argv[1] =='Yahoo':
            ...
            split1 = 1440
        cluster_value, _ = data_kpi.split_by_idx(split1, window_size=1)
        finetune_value, _ = data_kpi.split_by_idx(int(len(data_kpi.timestamps)*0.1), window_size=1)
        _, test_value = data_kpi.split_by_idx(int(len(data_kpi.timestamps)*0.5), window_size=1)
    ...
    for th in range(3,4):
        ...
        if sys.argv[1]=='Yahoo':
            kk=3
        tmp_result1 = {name: 0 for name in names}
        tmp_result2 = {name: 0 for name in names}
        all_score = np.array([])
        all_label = np.array([])
        ...
        for i, (ft, finetune, test_kpi, name) in enumerate(zip(cluster_feature, finetune_values, test_kpis, names)):
            ...
            all_score = np.append(all_score,anomaly_scores)
            all_label = np.append(all_label,test_kpi.labels)
            all_time = np.append(all_time,test_kpi.timestamps)
            all_value = np.append(all_value,test_kpi.values)
            all_missing = np.append(all_missing,test_kpi.missing)
            f1_score,pre,rec = get_test_results(
        ...
        delay_f1_score,d_pre,d_rec = get_delay_test_results(
                        timestamps=all_time,
                        labels=all_label,
                        scores=all_score,
                        missing=all_missing,
        ...
        max_f1_score,m_pre,m_rec = get_test_results(
                        timestamps=all_time,
                        labels=all_label,
                        scores=all_score,
                        missing=all_missing,
        ...
        with open('all_result.txt','a') as f:
            f.write('%f %f %f %f %f %f\n'%(max_f1_score,m_pre,m_rec,delay_f1_score,d_pre,d_rec))
...
if __name__ == '__main__':
    ...
    result1,result2  = main(2000)

    with open('{}_bestf1.json'.format(sys.argv[1]), 'w') as result_file:
        json.dump(result1, result_file)
    with open('{}_delayf1.json'.format(sys.argv[1]), 'w') as result_file:
        json.dump(result2, result_file)
    """ results = {'0':0.5}
    results = [results]
